...
const kDefaultReactions = <Reaction>[
  Reaction(key: 'like',  label: 'Like',  emoji: '👍', accent: Color(0xFF1877F2)),
  Reaction(key: 'love',  label: 'Love',  emoji: '❤️', accent: Color(0xFFE53935)),
  Reaction(key: 'care',  label: 'Care',  emoji: '🥰', accent: Color(0xFFFF6D00)),
  Reaction(key: 'haha',  label: 'Haha',  emoji: '😆', accent: Color(0xFFFFC107)),
  Reaction(key: 'wow',   label: 'Wow',   emoji: '😮', accent: Color(0xFFFFD54F)),
  Reaction(key: 'sad',   label: 'Sad',   emoji: '😢', accent: Color(0xFF64B5F6)),
  Reaction(key: 'angry', label: 'Angry', emoji: '😠', accent: Color(0xFFE65100)),
];
...
class ReactionButton extends StatefulWidget {
  ...
  final String? placeholderLabel;         // e.g. "Like"
  final String? placeholderEmoji;         // optional
  final String? placeholderAsset;         // outline like asset
  final Color?  placeholderAccent;        // text color when no reaction
...
class _ReactionButtonState extends State<ReactionButton> {
  ...
  Widget    build(BuildContext context) {
    ...
    final String label = isPlaceholder
        ? (widget.placeholderLabel ?? 'Like')
        : selected!.label;

    final Color accent = isPlaceholder
        ? (widget.placeholderAccent ?? theme.colorScheme.outline)
        : selected.accent;

    Widget icon;
    if (isPlaceholder) {
      if (widget.placeholderAsset != null) {
        icon = Image.asset(widget.placeholderAsset!, width: 30, height: 30, fit: BoxFit.contain);
      } else if (widget.placeholderEmoji != null) {
        icon = Text(widget.placeholderEmoji!, style: const TextStyle(fontSize: 20));
      } else {
        final r0 = widget.reactions.first;
        icon = (r0.asset != null)
            ? Image.asset(r0.asset!, width: 30, height: 30, fit: BoxFit.contain)
            : Text(r0.emoji ?? '👍', style: const TextStyle(fontSize: 20));
      }
    } else {
      final r0 = selected!;
      icon = (r0.asset != null)
  ...
  void _showOverlay() {
    ...
    final centerAbove = Offset(origin.dx + box.size.width / 2, origin.dy - 12);
  ...
  void show(BuildContext context) {
    _entry = OverlayEntry(
      ...
      builder: (ctx) => _ReactionOverlay(
        ...
        anchor: anchor,
        onSelected: (r) { _entry.remove(); onSelected(r); },
        onCancelled: () { _entry.remove(); onSelected(null); },
      ),
    );
...
class _ReactionOverlayState extends State<_ReactionOverlay>
  ...
  @override
  void initState() { super.initState(); _inCtrl.forward(); _bgCtrl.forward(); }
  @override
  void dispose() { _inCtrl.dispose(); _bgCtrl.dispose(); super.dispose(); }
  ...
  Rect _barRect(Size screen) {
    ...
    final left = (maxLeft >= minLeft)
        ? desiredLeft.clamp(minLeft, maxLeft)
        : minLeft;

    final top = math.max(safePad, widget.anchor.dy - widget.itemSize - 28);
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onPanDown: (d) => setState(() => _hoverIndex = _indexFor(d.globalPosition, _barRect(screen))),
      onPanUpdate: (d) => setState(() => _hoverIndex = _indexFor(d.globalPosition, _barRect(screen))),
      onPanEnd: (_) {
        if (_hoverIndex != null) {
          HapticFeedback.selectionClick();
...
class _ReactionBar extends StatelessWidget {
  const _ReactionBar({
    required this.reactions, required this.itemSize, required this.padding, required this.hoverIndex,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.15), blurRadius: 16, offset: const Offset(0, 10))],
        border: Border.all(color: isDark ? Colors.white10 : const Color(0xFFE6E6E6)),
      ),
      child: Row(
        children: List.generate(reactions.length, (i) {
          final r = reactions[i];
          final hovered = i == hoverIndex;
          return _ReactionItem(reaction: r, size: itemSize, hovered: hovered);
        }),
      ),
    );
  }
...
class _ReactionItem extends StatelessWidget {
  const _ReactionItem({required this.reaction, required this.size, required this.hovered});
  ...
  @override
  Widget build(BuildContext context) {
    final borderClr = hovered ? reaction.accent : Colors.transparent;

    final inner = (reaction.asset != null)
        ? Image.asset(reaction.asset!, width: size * 0.62, height: size * 0.62, fit: BoxFit.contain)
        : Text(reaction.emoji ?? '👍', style: TextStyle(fontSize: size * 0.6));
    ...
    return AnimatedContainer(
      ...
      transform: Matrix4.identity()..translate(0.0, hovered ? -8.0 : 0.0)..scale(hovered ? 1.18 : 1.0),
      decoration: BoxDecoration(
