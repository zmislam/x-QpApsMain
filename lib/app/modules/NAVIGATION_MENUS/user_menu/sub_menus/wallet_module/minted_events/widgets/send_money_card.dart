...
class SendMoneyCard extends StatelessWidget {
  final SendMoneyEventModel sendMoneyEventModel;
  final Function()? onMintPressed;

  const SendMoneyCard({
    super.key,
    required this.sendMoneyEventModel,
    this.onMintPressed,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final isMinted = sendMoneyEventModel.isSent ?? false;
    final hasRequestId = sendMoneyEventModel.requestId != null;
    final hasAmount = sendMoneyEventModel.amountInWei != null &&
        sendMoneyEventModel.amountInWei!.isNotEmpty;
    ...
    final requestIdText = hasRequestId
        ? _formatRequestId(sendMoneyEventModel.requestId.toString())
        : 'No Request ID';

    // Format amount if available
    final formattedAmount = hasAmount
        ? (num.parse(sendMoneyEventModel.amountInWei!)).toQPFlakes.toString()
        : 'Amount not available';

    return InkWell(
      onTap: () {
        Get.toNamed(Routes.QP_WALLET_TRANSACTION_VIEW,
            arguments: sendMoneyEventModel);
      },
      child: Card(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              Text(
                                getDynamicFormatedTime(sendMoneyEventModel
                                        .createdAt
                                        ?.toIso8601String() ??
                                    ''),
