...
class WithdrawEventCard extends StatelessWidget {
  final WithdrawnEventModel withdrawEventModel;
  final Function()? onMintPressed;

  const WithdrawEventCard({
    super.key,
    required this.withdrawEventModel,
    this.onMintPressed,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final isMinted = withdrawEventModel.isWithdrawn ?? false;
    final hasRequestId = withdrawEventModel.requestId != null;
    final hasAmount = withdrawEventModel.amountInWei != null &&
        withdrawEventModel.amountInWei!.isNotEmpty;
    ...
    final requestIdText = hasRequestId
        ? _formatRequestId(withdrawEventModel.requestId.toString())
        : 'No Request ID';

    // Format amount if available
    final formattedAmount = hasAmount
        ? (num.parse(withdrawEventModel.amountInWei!)).toQPFlakes.toString()
        : 'Amount not available';

    return InkWell(
      onTap: () {
        Get.toNamed(Routes.QP_WALLET_TRANSACTION_VIEW,
            arguments: withdrawEventModel);
      },
      child: Card(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              Text(
                                getDynamicFormatedTime(withdrawEventModel
                                        .createdAt
                                        ?.toIso8601String() ??
                                    ''),
