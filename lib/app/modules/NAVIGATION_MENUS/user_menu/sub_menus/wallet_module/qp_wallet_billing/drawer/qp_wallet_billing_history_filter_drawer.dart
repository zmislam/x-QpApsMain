...
class WalletBillingTransactionFilterDrawer extends StatefulWidget {
  const WalletBillingTransactionFilterDrawer({super.key});

  @override
  State<WalletBillingTransactionFilterDrawer> createState() =>
      _WalletBillingTransactionFilterDrawerState();
}

class _WalletBillingTransactionFilterDrawerState
    extends State<WalletBillingTransactionFilterDrawer> {
  final controller = Get.find<QpWalletBillingController>();
  ...
  Widget build(BuildContext context) {
    return Drawer(
      child: Padding(
        ...
        child: Column(
          children: [
            Text('Billing Filters'.tr,
                style: Theme.of(context).appBarTheme.titleTextStyle),
            const SizedBox(height: 10),

            // // $ ----------------------------------------------------------------------------------
            //
            // const FieldTitle(title: 'Transaction Type'.tr, isRequired: false),
            // Obx(
            //   () {
            //     if (!controller.walletManagementService.transactionBillTypeIsLoading.value) {
            //       return PrimaryDropDownFieldExtended<BillTypeModel>(
            //         hint: 'Select Transaction Type',
            //         items: controller.walletManagementService.transactionBillTypeList.value,
            //         onChanged: (value) {
