...
import '../drawer/qp_wallet_billing_history_filter_drawer.dart';

class QpWalletBillingView extends GetView<QpWalletBillingController> {
  const QpWalletBillingView({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      endDrawer: const WalletBillingTransactionFilterDrawer(),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                    ...
                    children: [
                      WalletAmountCardWithTitle(
                       ...
                       WalletAmountCardWithTitle(
                        ...
                        amount: '0', //controller.walletManagementService.walletSummary.value?.totalReceivedMoneyAmount?.toStringAsFixed(2) ?? '',
                      ),
                       WalletAmountCardWithTitle(
                        assetPath: AppAssets.SENT_MONEY_BALANCE_ICON,
                        title: 'Paid amount'.tr,
                        amount: '0', //controller.walletManagementService.walletSummary.value?.totalSendMoneyAmount?.toStringAsFixed(2) ?? '',
                      ),
                       WalletAmountCardWithTitle(
                        assetPath: AppAssets.WITHDRAW_BALANCE_ICON,
                        title: 'Total Due'.tr,
                        amount: '0', //controller.walletManagementService.walletSummary.value?.totalWithdrawRequestAmount?.toStringAsFixed(2) ?? '',
                      )
                    ],
                  ),
                ),
                ...
                Obx(
                  () => controller.billHistoryList.value.isEmpty
                      ?  SizedBox(
                          height: 200,
                          child: Center(
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            TransactionHistoryModel model = controller.billHistoryList.value[index];
