    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        when (call.method) {
            "init" -> {
                ...
                println("PRINTING APP METADATA THAT WAS RECEIVED");
                println("${appMetaData}");
                ...
                CoreClient.initialize(
                    ...
                    onError(
                        "init_core_error",
                        errorMessage = it.throwable.message ?: "ERROR FROM KT ---> INIT FAILED"
                    )
                }
            ...
            "connect" -> {
                val handleError: (Core.Model.Error) -> Unit = {
                    ...
                    println("ERROR IN CONNECTION SETUP --> FROM KT connect METHOD")
                }
                CoreClient.Relay.connect(onError = handleError)
                println("CONNECTION IS STABLISHED")
                result.success(null)
            }
            ...
            "getAllParing" -> {
                val listOfActivePairings: List<Core.Model.Pairing> =
                    CoreClient.Pairing.getPairings()
                println(listOfActivePairings)
                val resultList = listOfActivePairings.map { it.toFlutterValue() }
            ...
            "disconnectPairing" -> {
                ...
                CoreClient.Pairing.disconnect(
                    disconnect = Core.Params.Disconnect(
                        topic = topic,
                    ),
                    onError = {
                        onError(
                            "disconnect_pairing_error",
                            errorMessage = it.throwable.message ?: ""
                        )
                    }
                )
            ...
            "approve" -> {
                try {
                    ...
                    println("ERROR FROM APPROVAL METHOD ${e.message}")
                }
            }
            ...
            "createPair" -> {
                ...
                SignClient.connect(connectParams, {
                    ...
                    // ! BUG POINT --------------------
                    println("PAIRING HAS ERROR>> REJECTED");
                    result.success(null)
                    onError("create_pair_error", errorMessage = it.throwable.message ?: "")
                })
            }
...
fun Core.Model.Pairing.toFlutterValue(): Map<String, Any?> {
    return mapOf(
        "topic" to topic,
        "expiry" to expiry,
        "peerAppMetaData" to peerAppMetaData?.let {
            mapOf(
                "name" to it.name,
        ...
        "registeredMethods" to registeredMethods.split(",")
    )
}
